...
vector_t* _vector_init_ptr(vector_t vinit) {
    SAFE_MALLOC(vector_t*, vec, 1, sizeof(vector_t));

    *vec = vector_init(.length = vinit.length, .items = vinit.items, .capacity = vinit.capacity, .growth_factor = vinit.growth_factor);
